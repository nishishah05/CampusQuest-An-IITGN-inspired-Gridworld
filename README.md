...
Use the Arrow Keys on your keyboard or the Blue Buttons on the screen.

Guide the backpack 🎒 from the Hostel 🏠 to AB10 🏛️.

Tip: Avoid the 2D Lab (🍵) and Tea Post (☕) as they give negative rewards. Visit the Library (📚) for bonus points!
...
Click the orange "PLAY PATH" button.

The agent will execute the learned strategy step-by-step.

Victory: If the agent reaches AB10, enjoy the Mega Celebration with confetti, fanfare music, and a dancing victory popup! 💃🎉

Step 4: Customize the World (Edit Mode)

Want to make the game harder or change the layout?

Click the yellow "EDIT MAP" button. The Control Panel will flip to the Configuration Panel.

Move Walls: Click any empty cell on the grid to place a Wall (🧱). Click a wall to remove it.
...
Add New Landmarks: Use the "Landmark Builder" at the top of the panel to create custom stops (give it a name, an emoji, and a reward value).

Click "EXIT EDIT" to save your changes and reset the AI memory.
